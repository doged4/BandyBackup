...
def build_discriminator():
    ...
    model.add(LSTM(256, input_shape=img_shape, return_sequences=True))#img_rows)) #changed shape to rows
...
def save_imgs(generator, epoch):
   ...
   # noise = np.random.normal(0, 1, noise_len).reshape(100,)
    ...
    pickle.dump( gen_img, open("SuperNow2/"+(str(epoch) + ".p"), "wb" ) )
...
train(generator, discriminator, combined, epochs=5000001, batch_size=32, save_interval=50000)
